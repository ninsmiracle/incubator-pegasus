...
if [ "$_primary" == "true" ]; then
  echo "Set meta.lb._primary true"
  ...
  echo "remote_command -l $pmeta meta.lb._primary true" | ./run.sh shell --cluster $meta_list &>/tmp/$UID.$PID.pegasus.rebalance._primary
  set_ok=`grep OK /tmp/$UID.$PID.pegasus.rebalance._primary | wc -l`
  if [ $set_ok -ne 1 ]; then
